...
public class StudyCafePassMachine {
    ...
    private StudyCafeSeatPass selectPass() {
        ...
        List<StudyCafeSeatPass> passCandidates = findPassCandidatesBy(
            passType);

        return ioHandler.aksPassSelecting(passCandidates);
    }
    ...
    private Optional<StudyCafeLockerPass> selectedLockerPass(StudyCafeSeatPass selectedPass) {
        if(selectedPass.cannotUseLocker()){
            return Optional.empty();
        }

        Optional<StudyCafeLockerPass> lockerPassCandidate = findLockerPassCandidateBy(
            selectedPass);
